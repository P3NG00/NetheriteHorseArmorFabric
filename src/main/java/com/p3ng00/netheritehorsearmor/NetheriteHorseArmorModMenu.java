...
public class NetheriteHorseArmorModMenu implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return parent -> {
            ...
            ConfigCategory category = builder.getOrCreateCategory(createTranslatableText(null));
            TranslatableText requiresRestart = createTranslatableText("requires_restart");
            category.addEntry(builder.entryBuilder().startBooleanToggle(createTranslatableText("netherite_fire_resist_horse"), OPTION_NETHERITE_FIRE_RESIST_HORSE.value).setSaveConsumer(b -> OPTION_NETHERITE_FIRE_RESIST_HORSE.value = b).setDefaultValue(OPTION_NETHERITE_FIRE_RESIST_HORSE.defaultValue).build())
                    .addEntry(builder.entryBuilder().startBooleanToggle(createTranslatableText("netherite_fire_resist_player"), OPTION_NETHERITE_FIRE_RESIST_PLAYER.value).setSaveConsumer(b -> OPTION_NETHERITE_FIRE_RESIST_PLAYER.value = b).setDefaultValue(OPTION_NETHERITE_FIRE_RESIST_PLAYER.defaultValue).build())
